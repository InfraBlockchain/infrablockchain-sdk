...
impl<T: Types, D> WithNonce<T, D> {
    ...
    where
        ...
        T::BlockNumber: From<BlockNumberFor<T>>,
    {
    ...
    /// Returns `true` if given nonce is the next nonce for the given entity, i.e. is equal to
    /// current nonce plus 1.
    pub fn is_next_nonce(&self, nonce: T::BlockNumber) -> bool {
        Some(nonce) == self.next_nonce()
    }

    /// Returns mutable reference to the underlying data if provided nonce is equal to current nonce
    /// plus 1, otherwise returns an error.
    ...
    /// If supplied value is `Some(_)`, attempts to increase current nonce - succeeds if provided
    /// nonce is equal to current nonce plus 1, otherwise returns an error. If value is `None`,
