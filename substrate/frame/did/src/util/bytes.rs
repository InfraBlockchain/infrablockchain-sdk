...
/// wrapping arrays of size > 32. `$size` is the size of the underlying bytearray. Implements the
/// `Default`, `sp_std::fmt::Debug`, `PartialEq` and `Eq` trait as they will not be automatically
/// implemented for arrays of size > 32.
macro_rules! struct_over_byte_array {
	( $name:ident, $size:tt ) => {
		...
		#[scale_info(omit_prefix)]
		pub struct $name(
			#[cfg_attr(feature = "serde", serde(with = "hex::big_array"))] pub [u8; $size],
		);
