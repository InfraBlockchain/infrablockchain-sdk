...
/// All size limits for the `did-core` encodable data structures.
pub trait Limits: Clone + Eq {
    ...
    /// Maximum byte size of the accumulated value which is just one group element (not the number
    /// of members)
    ...
    /// Maximum byte size of the `BBS` (fixed size) public key. This depends only on the chosen
    /// elliptic curve.
    type MaxBBSPublicKeySize: Get<u32> + Send + Sync + 'static;
    /// Maximum byte size of the `BBS+` (fixed size) public key. This depends only on the chosen
    /// elliptic curve.
    type MaxBBSPlusPublicKeySize: Get<u32> + Send + Sync + 'static;
    /// Maximum byte size of the `PS` public key. This depends on the chosen elliptic curve and the
