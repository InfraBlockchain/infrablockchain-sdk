...
impl UncheckedDidKey {
    ...
    /// Constructs new `UncheckedDidKey` using given public key and all available verification
    /// relationships for this key.
    pub fn new_with_all_relationships(public_key: impl Into<PublicKey>) -> Self {
...
impl<T: Config> Pallet<T> {
    ...
    /// Return `did`'s key with id `key_id` only if has control capability, otherwise returns an
    /// error.
    ...
    /// Return `did`'s key with id `key_id` only if it can authenticate or control otherwise returns
    /// an error
