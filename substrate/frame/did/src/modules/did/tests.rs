...
fn offchain_did() {
    ...
    ext().execute_with(|| {
        ...
        assert_noop!(
            DIDModule::new_offchain(RuntimeOrigin::signed(alice), did, doc_ref.clone()),
        ...
        assert_noop!(
            DIDModule::new_offchain(RuntimeOrigin::signed(alice), did, doc_ref_1),
            Error::<Test>::DidAlreadyExists
        );
        ...
        assert_noop!(
            DIDModule::set_offchain_did_doc_ref(RuntimeOrigin::signed(bob), did, new_ref.clone()),
        ...
        assert_noop!(
            DIDModule::remove_offchain_did(RuntimeOrigin::signed(bob), did),
            Error::<Test>::DidNotOwnedByAccount
        );

        assert_ok!(DIDModule::remove_offchain_did(
            RuntimeOrigin::signed(alice),
            did
        ));
...
fn onchain_keyless_did_creation() {
    // Creating an on-chain DID with no keys but only controllers, i.e. DID is controlled by other
    // DIDs
    ext().execute_with(|| {
        let alice = 1u64;
...
fn onchain_keyed_did_creation_with_self_control() {
    ...
    ext().execute_with(|| {
        ...
        // DID controls itself when adding keys capable of signing without specifying any
        // verificatiion relationship
        assert_ok!(DIDModule::new_onchain(
            RuntimeOrigin::signed(alice),
            did_1,
        ...
        for pk in vec![PublicKey::sr25519(pk_sr), PublicKey::ed25519(pk_ed)] {
            for vr in vec![
                VerRelType::AUTHENTICATION,
                VerRelType::ASSERTION,
                VerRelType::CAPABILITY_INVOCATION,
        ...
        // Add single key with single relationship and but do not specify relationship as
        ...
        // Add single key, specify multiple relationships and but do not specify relationship as
        // `capabilityInvocation`
        assert_ok!(DIDModule::new_onchain(
            RuntimeOrigin::signed(alice),
            did_8,
...
fn batched_did_changes() {
    ...
    ext().execute_with(|| {
        ...
        // did_1 adds a key and service endpoint to itself in the same block. Checks that nonce is
        // correct.
        let add_keys = AddKeys {
            did: did_1,
            keys: vec![UncheckedDidKey::new(
