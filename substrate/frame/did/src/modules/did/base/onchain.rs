...
/// write (through an extrinsic). The nonce starts from the block number when the DID was created to
/// avoid replay attacks where an action of a DID that is removed and recreated by the same owner
/// and same key is replayed by someone else.
...
impl<T: Config> Pallet<T> {
    ...
    /// Try executing an action by a DID. Each action of a DID is supposed to have a nonce which
    /// should be one more than the current one. This function will check that payload has correct
    /// nonce and will then execute the given function `f` on the action and if `f` executes
    /// successfully, it will increment the DID's nonce by 1.
        ...
        E: From<Error<T>> + From<NonceError>,
    {
        ...
        if action.target() != *signature.did {
            ...
            // Target DID and acting (signer) DID are different and thus both DID's details must be
            // modified
