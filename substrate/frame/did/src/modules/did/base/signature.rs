...
impl<D: Into<Did>> DidSignature<D> {
    ...
    /// This is just the weight to verify the signature. It does not include weight to read the DID
    /// or the key.
...
impl<T: Config> Pallet<T> {
    /// Verifies a `DidSignature` created by `signer` only if `signer` is a controller of `did` and
    /// has an appropriate key. To update a DID (add/remove keys, add/remove controllers), the
    /// updater must be a controller of the DID and must have a key with `CAPABILITY_INVOCATION`
    ...
    /// Verifies that `did`'s key with id `key_id` can either authenticate or control otherwise
    /// returns an error. Then provided signature will be verified against the supplied public key
    /// and `true` returned for a valid signature.
