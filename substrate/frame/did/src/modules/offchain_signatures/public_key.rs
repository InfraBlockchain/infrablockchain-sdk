...
/// Public key for different signature schemes. Currently can be either `BBS`, `BBS+` or
/// `Pointcheval-Sanders`.
#[derive(
...
impl<T: Limits> OffchainPublicKey<T> {
    ...
    /// Returns underlying **unchecked** bytes representation for a key corresponding to either
    /// signature scheme.
