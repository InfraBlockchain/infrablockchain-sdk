...
impl<T: Config> Pallet<T> {
    ...
    /// Executes action over target `StatusListCredential` providing a mutable reference if all
    /// checks succeed.
    ///
    /// Checks:
    ...
    ///
    /// Returns a mutable reference to the underlying StatusListCredential if the command is
    ...
    /// checks succeed.
    ///
    /// Unlike `try_exec_action_over_status_list_credential`, this action may result in a removal of
    /// a `StatusListCredential`, if the value under option will be taken.
    ///
    /// Checks:
    /// 1. Ensure that the `StatusListCredential` exists.
    /// 2. Verify that `proof` authorizes `action` according to `policy`.
    ...
    /// nonce equals the last nonce plus 1.
    ///
    /// Returns a mutable reference to the underlying `StatusListCredential` wrapped into an option
    /// if the command is authorized, otherwise returns Err.
