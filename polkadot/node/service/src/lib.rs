...
pub use chain_spec::{GenericChainSpec, YosemiteChainSpec};
...
pub use {yosemite_runtime, yosemite_runtime_constants};
...
impl IdentifyVariant for Box<dyn ChainSpec> {
	...
	fn is_yosemite(&self) -> bool {
		self.id().starts_with("yosemite") || self.id().starts_with("yos")
	}
	fn is_dev(&self) -> bool {
			...
			Some(CandidateValidationConfig {
				...
				pvf_execute_workers_max_num: execute_workers_max_num.unwrap_or_else(
					|| match config.chain_spec.identify_chain() {
						...
						Chain::Yosemite | Chain::Unknown => 4,
					},
				),
