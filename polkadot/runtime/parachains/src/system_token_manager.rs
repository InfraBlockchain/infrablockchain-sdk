...
use sp_std::prelude::*;
pub use traits::{OracleInterface, SystemTokenInterface};
use types::*;
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config + configuration::Config + paras::Config {
		...
		// TODO: Generics
		type HigherPrecision: SystemTokenWeight<
...
impl<T: Config> Pallet<T> {
	...
	fn calc_system_token_weight(
		...
		let exchange_rate_to_base =
			ExchangeRates::<T>::get(currency).ok_or(Error::<T>::ExchangeRateNotRequested)?;
		let BaseSystemTokenDetail { base_weight, base_decimals, .. } =
			configuration::Pallet::<T>::active_system_config().base_system_token_detail;
...
pub mod traits {
	...
	impl SystemTokenInterface for () {
		...
		fn register_system_token(
			...
			_asset_id: Self::Location,
			_system_token_weight: Self::SystemTokenWeight,
		) {
		}
		fn deregister_system_token(_dest_id: Self::DestId, _asset_id: Self::Location) {}
		fn create_wrapped(
			_dest_id: Self::DestId,
			_owner: Self::AccountId,
			_original: Self::Location,
			_currency_type: Fiat,
		...
		fn suspend_system_token(_dest_id: Self::DestId, _asset_id: Self::Location) {}
		fn unsuspend_system_token(_dest_id: Self::DestId, _asset_id: Self::Location) {}
	}
	...
	impl OracleInterface for () {
		type DestId = u32;
		fn request_fiat(_dest_id: Self::DestId, _fiat: Vec<Fiat>) {}
	}
}
