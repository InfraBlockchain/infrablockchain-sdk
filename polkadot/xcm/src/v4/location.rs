...
impl SystemTokenId for Location {
	type AssetId = u128;
	type PalletId = u8;
	type OriginId = u32;
	...
	fn id(&self) -> Result<(Option<Self::OriginId>, Self::PalletId, Self::AssetId), Self::Error> {
		match self.interior.clone() {
			...
			Junctions::X2(xs) => {
				let [a, b] = *xs;
				match (a, b) {
					(Junction::PalletInstance(pallet_id), Junction::GeneralIndex(asset_id)) =>
	...
	fn convert_back(
		...
		if let Some(para_id) = origin_id {
			...
			Location::new(
				0,
				Junctions::X2(
					[Junction::PalletInstance(pallet_id), Junction::GeneralIndex(asset_id)].into(),
				),
			)
		}
	}
	...
	fn reanchor_loc(
		...
		let target = if let Some(dest_id) = maybe_dest_id {
			Location::new(parents, Junctions::X1([Junction::Parachain(dest_id)].into()))
		} else {
			Location::new(parents, Junctions::Here)
		};
		self.reanchor(&target, context).map_err(|_| Self::Error::ErrorOnReanchor)?;
		Ok(())
	}
